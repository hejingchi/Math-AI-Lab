\section{半无穷区间上的推广：Baskakov 与 Mirakyan 算子}

\subsection{Baskakov 线性正算子序列}
...
\]
从组合角度看，第二步到第三步相当于从 $n$ 个独立的几何分布随机变量中抽取 $k$ 个成功的方法数。$\binom{n+k-1}{k}$ 计数了在 $n$ 个位置上放置 $k$ 个成功的方式，这正是负二项分布概率质量函数的核心组成部分。基于此，我们通过对 $z$ 求导建立三个关键引理。
...
Mirakyan 算子同样是 Korovkin 定理的一个特例。