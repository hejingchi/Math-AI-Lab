...
\item 不等式约束的系数 $\alpha_i \geqslant0$
\item 目标函数的极值要么在不等式约束的边界上取到等号，也就是 $c_i(x)=0$，此时 $\alpha_i>0$。
...
定义
\begin{equation}
\theta_p(x) = \max_{\alpha,\beta,\alpha_i\geqslant0}
L(x, \alpha, \beta)
\end{equation}
如果 $x$ 满足约束条件，即
\begin{equation}
...
\subsection{广义拉格朗日的极大极小问题}

定义
\begin{equation}
\theta_d(\alpha, \beta) = \min_{x}
L(x, \alpha, \beta)
\end{equation}
如果我们在这里考虑
\begin{equation}
...
令
\begin{equation}
d^* =\max_{\alpha,\beta,\alpha_i\geqslant0}\min_xL(x,\alpha,\beta)
\end{equation}
是对偶问题的解。
...
\begin{itemize}
\item 第一条称为驻点条件（stationarity）
\item 第二、三条称为原始可行性（primal feasibility）
\item 第四条称为对偶可行性（dual feasibility）
\item 最后一条称为互补松弛条件（complementary slackness）
\end{itemize}
在凸优化问题中，当满足适当条件时（例如 Slater 条件），原问题与对偶问题满足强对偶性，此时 KKT 条件也是最优解的充分必要条件。

\subsection{强对偶与KKT条件的联系}
...
\nabla_x L(x^*,\alpha^*,\beta^*) = 0
\end{equation}
\textbf{第二个等号}：$L(x^*,\alpha^*,\beta^*) = f(x^*)$
展开就是
\begin{equation}
\sum_i \alpha_i^* c_i(x^*) = 0
\end{equation}
...
从而我们如果知道一个优化问题是强对偶的，那么他的最优解就一定满足 KKT 条件。

反过来，如果 $x^*$ 满足 KKT 条件，我们想问它是否就是最优解呢？答案一般是否定的。只有对于凸优化问题而言，才能用满足 KKT 
条件的点得到这个点就是最优解。

因此，\textbf{KKT条件是强对偶成立时最优解必须满足的条件}，它从不等式链取等这一事实中自然导出，
...
\subsection{对对偶问题的进一步理解}
 
现在来说明，对偶问题的求解过程中，$\nabla_x L = 0$ 这一步究竟是什么意思。
 
\textbf{第一层含义：求对偶函数的计算工具}
...
\end{equation}
并利用对偶问题的解来求原问题的解。为了求 $\theta_d(\alpha)$ 的表达式，
令 $L$ 对 $w$ 和 $b$ 的偏导为零
\begin{equation}
